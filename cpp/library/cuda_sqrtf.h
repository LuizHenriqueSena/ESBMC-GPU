...
fp_t sqrtf(fp_t x) {
    ...
    // Various constants parametrized by the type of x:
     int typeWidth = sizeof(rep_t) * 8;
     int exponentBits = typeWidth - significandBits - 1;
     int exponentBias = (1 << (exponentBits - 1)) - 1;
     rep_t minNormal = REP_C(1) << significandBits;
     rep_t significandMask = minNormal - 1;
     rep_t signBit = REP_C(1) << (typeWidth - 1);
     rep_t absMask = signBit - 1;
     rep_t infRep = absMask ^ significandMask;
     rep_t qnan = infRep | REP_C(1) << (significandBits - 1);
