...
ESBMC-GPU is built on top of ESBMC, which is an open source context-bounded model checker based on SMT solvers for ANSI-C/C$++$ programs~\cite{cordeiro:2011,cordeiro:2012}, and adds four essential models, as described below.
...
\noindent \textbf{2. Two-threads Analysis.} Similarly to GPUVerify~\cite{betts:2012} and PUG~\cite{Li:2010}, ESBMC-GPU also reduces the number of threads (to only two elements), during the verification of CUDA programs, by considering a NVIDIA Fermi GPU architecture, in order to improve verification time and avoid the state-space explosion problem. Besides, in CUDA programs, whilst threads execute the same parametrized kernel, only two of them are necessary for conflict check. Thus, such an analysis ensures that errors ({\it e.g.}, data races) detected between two threads, in a given subgroup and due to unsynchronized accesses to shared variables, are enough to justify a property violation~\cite{pereira:2016}.
...
In addition, ESBMC-GPU still presents the highest accuracy, with less than $6$ seconds per benchmark.
