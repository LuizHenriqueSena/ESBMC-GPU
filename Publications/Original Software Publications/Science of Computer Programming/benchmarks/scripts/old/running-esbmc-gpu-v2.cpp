...
#define ESBMC_GPU_RUN_COMMAND_BASE "esbmc-gpu main.cu "
#define LIBRARIES_INPUT_PARAMETER "-I ~/libraries/" 
...
string get_run_command()
{
    ...
    while(!(test_description.eof()))
    {
       ...
       if (line.find(LIBRARIES_INPUT_PARAMETER) != string::npos)
       {
          command = line;
          break;
       }
    ...
    return ESBMC_GPU_RUN_COMMAND_BASE + command;
}
...
void run_esbmc_gpu(unsigned int &correct, unsigned int &false_correct, unsigned int &false_incorrect,
                   unsigned int &incorrect, unsigned int &not_supported)
{
    auto successful = true;
    auto run_command_output = execute_command(get_run_command().c_str());
    for (const auto &message : run_command_output)
    {
        if (message.find(VERIFICATION_FAILED_MESSAGE) != string::npos)
        {
            successful = false;
            break;
        }
...
void calculate_verification_rate()
{
    ...
    for (auto suite = suites.begin(); suite != suites.end(); ++suite)
    {
        ...
        for (auto sub_suite = sub_suites.begin(); sub_suite != sub_suites.end(); ++sub_suite) {
            ...
            for (auto test_case = test_cases.begin(); test_case != test_cases.end(); ++test_case) {
                ...
                run_esbmc_gpu(correct, false_correct, false_incorrect, incorrect, not_supported);
                cout << " ---- " << *test_case << endl;
                chdir("../.");
            }
            ...
            inner_elapsed += (inner_end.tv_sec - inner_start.tv_sec) + (inner_end.tv_nsec - inner_start.tv_nsec) / BILLION;
            cout << "TIME: " << inner_elapsed << "s" << endl;
            cout << "=====================================" << endl;
            test_cases.clear();
            cout << endl;
            chdir("../.");
    ...
    double elapsed = chrono::duration_cast<chrono::nanoseconds>(end_steady - start_steady).count() / BILLION;
    elapsed += (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / BILLION;
    suites.clear();
    cout << "==========================================" << endl;
