...
#define GPU_VERIFY_COMMAND "gpuverify main.cu --blockDim=2 --gridDim=1"
...
void run_gpuverify(unsigned int &correct, unsigned int &false_correct, unsigned int &false_incorrect,
                   unsigned int &incorrect, unsigned int &not_supported)
{
    auto successful = true;
    auto run_command_output = execute_command(GPU_VERIFY_COMMAND);
    ...
    for (auto suite = suites.begin(); suite != suites.end(); ++suite)
    {
        ...
        for (auto sub_suite = sub_suites.begin(); sub_suite != sub_suites.end(); ++sub_suite) {
            ...
            for (auto test_case = test_cases.begin(); test_case != test_cases.end(); ++test_case) {
                ...
                run_gpuverify(correct, false_correct, false_incorrect, incorrect, not_supported);
                cout << " ---- " << *test_case << endl;
                chdir("../.");
            }
