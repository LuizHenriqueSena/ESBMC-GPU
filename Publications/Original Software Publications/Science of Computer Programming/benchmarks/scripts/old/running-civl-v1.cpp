...
#define CIVL_COMMAND "civl verify main.cu"
...
void run_civl(unsigned int &correct, unsigned int &false_correct, unsigned int &false_incorrect,
                   unsigned int &incorrect, unsigned int &not_supported)
{
    auto successful = false, supported = false;
    auto run_command_output = execute_command(CIVL_COMMAND);
    for (const auto &message : run_command_output)
    {
        if (!supported && message.find(RESULT_DESCRIPTION) != string::npos)
            supported = true;
        if (message.find(PROPERTY_HOLDS) != string::npos)
        {
            successful = true;
            break;
        }
    ...
    for (auto suite = suites.begin(); suite != suites.end(); ++suite)
    {
        ...
        for (auto sub_suite = sub_suites.begin(); sub_suite != sub_suites.end(); ++sub_suite) {
            ...
            for (auto test_case = test_cases.begin(); test_case != test_cases.end(); ++test_case) {
                ...
                run_civl(correct, false_correct, false_incorrect, incorrect, not_supported);
                cout << " ---- " << *test_case << endl;
                chdir("../.");
            }
